...
def match_nombre_riguroso(nombre_buscado, nombre_candidato):
    ...
    # 1. Match Exacto (Si son idénticos, aprobar inmediatamente)
    ...
    # 3. VERIFICACIÓN DE NOMBRE DE PILA (CRUCIAL)
    # El primer nombre (token 0) DEBE estar presente.
    # Esto evita que "Andrés Fuentes" haga match con "Juan Fuentes".
    if tokens_buscado and tokens_candidato:
        primer_nombre = nombre_norm_buscado.split()[0]
        if primer_nombre not in tokens_candidato:
            return False

    # 4. VERIFICACIÓN ESTRICTA DE APELLIDOS
    # Extraemos los probables apellidos (últimas 2 palabras)
    apellidos_buscados = extraer_tokens_apellidos(nombre_buscado)
    
    # Verificamos que TODOS los apellidos buscados estén en el candidato.
    # Esto evita que "Fuentes Castillo" haga match con "Pérez Castillo".
    apellidos_encontrados = 0
    for ap in apellidos_buscados:
        if ap in tokens_candidato:
            apellidos_encontrados += 1
            
    # Si detectamos 2 apellidos en la búsqueda, exigimos que ambos aparezcan
    if len(apellidos_buscados) > 1:
        if apellidos_encontrados < len(apellidos_buscados):
            return False
    # Si solo detectamos 1 apellido, exigimos que ese aparezca
    elif len(apellidos_buscados) == 1:
        if apellidos_encontrados < 1:
            return False
...
repo_url = "https://github.com/Benjapoitiers/anid_usm"

with st.sidebar:
    st.image("https://upload.wikimedia.org/wikipedia/commons/4/47/Logo_UTFSM.png", width=100) # Opcional: Logo en sidebar
    st.write("---")
            ...
            if df_corpus is not None:
                ...
                if not final_user_text.strip():
                    ...
                    if len(sorted_indices) > 0:
                        ...
                        # Calculamos similitud visual de los nombres (0 a 1)
                        # Esto ayuda a conectar "Patricio A. Catalan" con "Patricio Andres Catalan..."
                        name_similarity = SequenceMatcher(None, input_clean, top_clean).ratio()
                        
                        # REGLAS DE DETECCION MEJORADAS:
                        # 1. Score extremo (> 0.95): Es la misma persona casi seguro (mismos papers).
                        # 2. Match Riguroso Y Score alto (> 0.8): La lógica que ya tenías.
                        # 3. Nombre visualmente similar (> 0.6) Y Score decente (> 0.6): Esta regla arregla tu caso.
                        
                        if top_score > 0.95 or \
                           (match_nombre_riguroso(input_clean, top_clean) and top_score > 0.8) or \
                           (name_similarity > 0.6 and top_score > 0.6):
                           
                            is_internal_user = True
                            self_match_score = top_score
                            self_match_name = top_name
                            # Quitamos al propio usuario (índice 0) de la lista
